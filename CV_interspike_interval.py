...
for r in input_r:
    ...
    for g in gvariable:
        ...
        for ii in range(parameters_sim.n_e_23):
            train_23_e = np.load(f'{base_path}\\Spiketraintest_23e_g=%g_r=%g.npy'%(g,r),
        ...
        for ii in range(parameters_sim.n_i_23):
            train_23_i = np.load(f'{base_path}\\Spiketraintest_23i_g=%g_r=%g.npy'%(g,r),
                             allow_pickle=True)[0][ii]
        ...
        for ii in range(parameters_sim.n_e_4):
            train_4_e = np.load(f'{base_path}\\Spiketraintest_4e_g=%g_r=%g.npy'%(g,r),
                            allow_pickle=True)[0][ii]
            cv_4_e = CV(train_4_e)
            if cv_4_e != cv_4_e:
                print('nan e')
        ...
        for ii in range(parameters_sim.n_i_4):
            train_4_i = np.load(f'{base_path}\\Spiketraintest_4i_g=%g_r=%g.npy'%(g,r),
                            allow_pickle=True)[0][ii]
            cv_4_i = CV(train_4_i)
            if cv_4_i != cv_4_i:
                print('nan i')
...
f = open(f'{base_path}\\cv_values.pckl', 'wb')
